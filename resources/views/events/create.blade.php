...
<body class="container mx-auto my-10 bg-slate-800">
    ...
    <div  >
        <h1 class="text-orange-500 text-5xl mb-14 text-center">Crie sua tarefa</h1>
        <form action="/events" method="POST" class="flex flex-col gap-4 px-5">
            @csrf
            <div class="flex flex-col gap-2">
                <label class="text-orange-400 font-semibold text-lg md:text-xl" for="title">Tarefa: </label>
                <input type="text" class="bg-slate-300 h-14 pl-7 rounded-2xl" id="title" name="title" placeholder="Nome da tarefa" maxlength="23" required/>
            </div>
            <div class="flex flex-col md:flex-row my-10">
                <div class="flex flex-row mb-5 md:mb-0 items-center md:mr-10">
                    ...
                    <input type="date" class="bg-slate-300 md:h-14 h-10 px-7 ml-5 rounded-xl md:rounded-2xl md:w-48" id="city" name="city" required/>
                </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea name="description" id="description" class="bg-slate-200 pl-7 pt-4 h-20 mt-0 rounded-2xl" placeholder="Descreva a tarefa" required></textarea>
            </div>
            <input class="mt-5 bg-orange-500 text-white font-semibold rounded-2xl text-lg md:text-xl p-4 items-center text-center" type="submit" value="Criar Tarefa">
